...
def recognize(encoding_data, rgb_image):
    ...
    for encoding in encodings:
        ...
        matches = face_recognition.compare_faces(encoding_data[constants.ENCODING_DATA], encoding, tolerance=0.6)
        name = "Unknown"

        faceDistance = face_recognition.face_distance(encoding_data[constants.ENCODING_DATA], encoding)

        # Tìm distance nhỏ nhất và index của nó
        minDistance = 1
        for index in range(len(faceDistance)):
            if faceDistance[index] < minDistance:
                minIndex, minDistance = index, faceDistance[index]

        # Nếu minDistance thoả mãn điều kiện tolerance <= 0.4
        if matches[minIndex]:
            return [encoding_data[constants.ENCODING_NAME][minIndex]]
